...
public class DBManager {
	...
	public static boolean titleExists(String title) throws SQLException {
		...
		try {
			...
			throw new SQLException("There was an error trying to lookup a user: " + title + ".", ex);
		}
		
		return retVal;
	}
	
	public static boolean createTitle(String title) throws SQLException {
		boolean retVal = false;
		
		PreparedStatement ps = null;
		
		try {
			ps = SQLDatabase.dbm.getConnection().prepareStatement("INSERT INTO titles (titlename) VALUES (?);");
			ps.setString(1, title);
	...
	public static boolean deleteTitle(String title) throws SQLException {
		boolean retVal = false;
		
		PreparedStatement ps = null;
		
		try {
			ps = SQLDatabase.dbm.getConnection().prepareStatement("DELETE FROM titles WHERE titlename = ?;");
			ps.setString(1, title);
			retVal = ps.executeUpdate() > 0;
			
			ps.close();
		} catch (SQLException ex) {
			throw new SQLException("There was an error trying to delete a title: " + title + ".", ex);
		}
	...
	public static String getTitleFromId(int id) throws SQLException {
		...
		try {
			...
			throw new SQLException("There was an error trying to retrieve title with id: " + id + ".", ex);
		}
		
		return titleName;
	}
	
	public static boolean grantTitle(String playerName, int titleId) throws SQLException {
		boolean retVal = false;
	...
	public static boolean revokeTitle(String playerName, int titleId) throws SQLException {
		boolean retVal = false;
	...
	public static boolean hasTitle(String playerName, int titleId) throws SQLException {
		boolean retVal = false;
